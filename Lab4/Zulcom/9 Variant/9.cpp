...
/**
 * функция setNegativeZero принимает массив и его размер, ищет в нём числа меньше 0 и зануляет их
 */
void setNegativeZero(int*A, int M)
{
	for(int i = 0; i < M; i++) // бежим по циклу
		if(A[i] <0) // если число меньше 0
			A[i] = 0; // присваиваем ему 0
}
/**
...
{
	ifstream in(s, ios::binary); // открываем sample.bin на чтение используя поточный метод чтения из библиотеки fstream. То, что вывод будет именно двоичным, определяет флаг binary из пространства имён ios
	...
	for(int i = 0; i < N; i++) // бежим по строкам двоичного файла
	{
		...
		for(int j = 0; j < M; j++) // теперь пробегаем по этому массиву и выводим каждый элемент через пробел
			cout << ioBuffer[j] << " "; // выводм на экран все числа в массиве через пробел
...
{
	ifstream binIn("sample.bin", ios::binary);   // открываем sample.bin на чтение используя поточный метод чтения из библиотеки fstream. То, что вывод будет именно двоичным,
	...
	// определяет флаг binary из пространства имён ios ( begin - начало на английском языке).
...
void insertRowBin(int*A, int k)
{
	...
	ofstream out("UpBin.bin", ios::binary); // открываем UpBin.bin на чтение используя поточный метод чтения из библиотеки fstream. То, что вывод будет именно двоичным, определяет флаг binary из пространства имён ios
	if(!in.is_open() || !out.is_open()) cout << "sample.bin cant open for read.";  // если файл по каким-то причинам не открыт, выводим об этом сообщение.
	int N, M; // две переменных, в которых мы будем временно хранить размерность матрицы
	in.read(reinterpret_cast<char*>(&N), sizeof(N)); //cм. 63-66.
	in.read(reinterpret_cast<char*>(&M), sizeof(M));
	out.write(reinterpret_cast<char*>(&N), sizeof(N)); // запишем размерность в новый файл
	out.write(reinterpret_cast<char*>(&M), sizeof(M));

	int* ioBuffer = new int[M];// создадим массив размера M для хранения каждой строки из текстового файла
	for(int i = 0; i < N; i++) // пробегаемся по строкам
	{
		if(k == i) ioBuffer = A; // если строка та, что нужно заменить, то мы записываем в текущую строку этот массив
		else
			in.read(reinterpret_cast<char*>(ioBuffer), sizeof(ioBuffer[0])*M); // иначе просто считываем её с файла
		out.write(reinterpret_cast<char*>(ioBuffer), sizeof(ioBuffer[0])*M); // записываем строку во второй двоичный файл.
	}
	delete[] ioBuffer; // очистим память временного массива
	in.close(); // закроем покток чтения из тектового файла
	out.close();// закрываем поток записи в двоичный файл
}
int main()
{
	...
	printFromBin("sample.bin"); // выводим на экран сождержимое двоичного файла
	...
	setNegativeZero(kRow, dementions[1]); // используем функцию по заданию
	insertRowBin(kRow, k - 1); // вставляем на старое место новую строку
	printFromBin("UpBin.bin"); // печатаем из нового файла
